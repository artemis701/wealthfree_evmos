...
Find below a list of `wealthfreed` commands added with the `x/vesting` module. You can obtain the full list by using the `wealthfreed -h` command.
...
```go
wealthfreed add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
wealthfreed query vesting balances ADDRESS [flags]
```
...
```go
wealthfreed tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
wealthfreed tx vesting clawback ADDRESS [flags]
```
...
```go
wealthfreed tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
