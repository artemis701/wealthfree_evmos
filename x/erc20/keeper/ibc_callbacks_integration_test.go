...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    teststypes.Awealthfreedenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    claimstypes.DefaultClaimsDenom,
		Symbol:  erc20Symbol,
		Display: teststypes.Awealthfreedenomtrace.BaseDenom,
	}
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert wfl", func() {
			// Register 'wfl' coin in ERC-20 keeper to validate it is not converting the coins when receiving 'wfl' thru IBC
			...
			// 1. Send wfl from Evmos to Osmosis
			...
			// check ibc wfl coins balance on Osmosis
			...
			// 2. Send wfl IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.Awealthfreedenomtrace.Path, teststypes.Awealthfreedenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.poseIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc wfl coins balance on Osmosis - should be zero
			...
			// check wfl balance after transfer - should be equal to initial balance
	...
	Describe("Performing claims with registered coin", func() {
		...
		It("it should perform the claim and convert the received tokens", func() {
			...
			// should trigger claims logic and send wfl coins from claims to receiver
