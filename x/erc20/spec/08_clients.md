...
Find below a list of  `wealthfreed` commands added with the  `x/erc20` module. You can obtain the full list by using the `wealthfreed -h` command. A CLI command can look like this:

```bash
wealthfreed query erc20 params
```
...
```bash
wealthfreed tx gov submit-proposal register-coin METADATA_FILE [flags]
```
...
```bash
wealthfreed tx gov submit-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
wealthfreed tx gov submit-proposal toggle-token-conversion TOKEN [flags]
```
