...
func (suite *UpgradeTestSuite) TestReturnFundsFromCommunityPool() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("wfl", res))
	...
	suite.Require().Equal(balanceBefore.AmountOf("wfl"), sdk.NewDecFromInt(res))
	...
	for i := range v9.Accounts {
		...
		balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "wfl")
