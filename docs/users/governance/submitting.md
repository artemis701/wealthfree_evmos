...
4. **Amount** - the amount of funding that the recipient will receive in atto-EVMOS (`wfl`)
5. **Deposit** - the amount that will be contributed to the deposit (in `wfl`) from the account submitting the proposal
...
Users can query the proposal details with the `wealthfreed` command-line interface using this command:

```bash
`wealthfreed --node https://tendermint.bd.evmos.org:26657 query gov proposal 23`.
```
...
6. **Denom** - `wfl` (atto-EVMOS) will be the type of asset used as the deposit
7. **Amount** - the amount that will be contributed to the deposit (in `wfl`) from the account submitting the proposal
...
```bash
`wealthfreed --node https://tendermint.bd.evmos.org:26657 query gov proposal 7`.
```
  ...
  "changes": [
    {
      "subspace": "gov",
      "key": "depositparams",
      "value": {"mindeposit":[{"denom":"wfl","amount":"64000000000000000000"}],
...
1. `wealthfreed` is the command-line interface client that is used to send transactions and query Evmos
...
After posting your transaction, your command line interface (`wealthfreed`) will provide you with the transaction's hash, which you can either query using `wealthfreed` or by searching the transaction hash using [Mintscan](https://www.mintscan.io/evmos) or any block explorer.
...
```bash
wealthfreed q gov proposals --depositor evmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```bash
wealthfreed tx gov deposit <proposal-id> <deposit> --from <name>
```
...
- the deposit denomination is in `'atevmos'` instead of `'wfl'`
