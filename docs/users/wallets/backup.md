...
```bash
$ wealthfreed keys add dev0-restored --recover
...
```bash
wealthfreed keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ wealthfreed keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ wealthfreed keys unsafe-import-eth-key dev0-imported ./dev0.export
