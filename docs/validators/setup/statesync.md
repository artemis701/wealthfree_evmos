...
```bash
wealthfreed init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .wealthfreed/config folder

```bash
mv $HOME/genesis.json ~/.wealthfreed/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.wealthfreed/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.wealthfreed/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.wealthfreed/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.wealthfreed/config/config.toml
```

### Create wealthfreed service

```bash
echo "[Unit]
Description=wealthfreed Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which wealthfreed) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/wealthfreed.service; sudo mv $HOME/wealthfreed.service /etc/systemd/system/
```

```bash
sudo systemctl enable wealthfreed.service && sudo systemctl daemon-reload
```

### Run wealthfreed

```bash
sytemctl start wealthfreed
```

### Check logs

```bash
journalctl -u wealthfreed -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.wealthfreed/config/config.toml
```
