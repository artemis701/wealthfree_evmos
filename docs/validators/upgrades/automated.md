...
echo "export DAEMON_NAME=wealthfreed" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.wealthfreed" >> ~/.profile
source ~/.profile
```

After this, you must make the necessary folders for `cosmosvisor` in your `DAEMON_HOME` directory (`~/.wealthfreed`) and copy over the current binary.

```bash
mkdir -p ~/.wealthfreed/cosmovisor
mkdir -p ~/.wealthfreed/cosmovisor/genesis
mkdir -p ~/.wealthfreed/cosmovisor/genesis/bin
mkdir -p ~/.wealthfreed/cosmovisor/upgrades

cp $GOPATH/bin/wealthfreed ~/.wealthfreed/cosmovisor/genesis/bin
```

To check that you did this correctly, ensure your versions of `cosmovisor` and `wealthfreed` are the same:
...
```bash
sudo tee /etc/systemd/system/wealthfreed.service > /dev/null <<EOF
[Unit]
...
Environment="DAEMON_HOME=$HOME/.wealthfreed"
Environment="DAEMON_NAME=wealthfreed"
...
sudo -S systemctl enable wealthfreed
sudo -S systemctl start wealthfreed
```

You can check the status with:

```bash
systemctl status wealthfreed
```
